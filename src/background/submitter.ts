 ...
 */
export async function submitSubmission(payload: SubmissionPayload): Promise<void> {
  ...
  if (success) {
    ...
    log.info('Submission synced successfully to GitHub');
  } else {
    await incrementCounter('totalFailed');

    await addToFailedQueue(payload);

    log.warn('Submission added to retry queue');
  }
}
...
export async function retryFailed(): Promise<number> {
  ...
  if (failedQueue.length === 0) {
    log.info('No failed submissions');
  ...
  for (const submission of failedQueue) {
    const success = await attemptSendWithRetries(submission);
 ...
 * Retry wrapper
 */
async function attemptSendWithRetries(payload: SubmissionPayload): Promise<boolean> {
  for (let attempt = 0; attempt < RETRY_CONFIG.MAX_RETRIES; attempt++) {
    if (attempt > 0) {
      ...
      log.info(`Retry ${attempt + 1}/${RETRY_CONFIG.MAX_RETRIES} after ${delay}ms`);

      await sleep(delay);
    }

    try {
      const result = await sendSubmissionToGithub(payload);
      ...
      await setStorage({
        lastError: result.message ?? 'GitHub sync failed',
      });
    } catch (error) {
